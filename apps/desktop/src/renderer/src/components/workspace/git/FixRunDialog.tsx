...
export function FixRunDialog({
  ...
  // user has read it. Edits after that mark the sizing stale; the chip offers a re-run.
  ...
  const suggestion =
    ...
    recommendation.recommendation.profile.cliId &&
    recommendation.args.length > 0
      ? {
          cliId: recommendation.recommendation.profile.cliId,
          args: recommendation.args,
          label: [
            recommendation.choice.model.label,
            recommendation.choice.effort ? EFFORT_LABELS[recommendation.choice.effort] : null,
          ]
            .filter(Boolean)
            .join(' · '),
        }
  ...
  async function openFix(launch: {
    ...
    try {
      ...
      const tabId = await launchPromptTab(project, { ...launch, prompt }, group?.id);
      if (!tabId) return;
      onOpenChange(false);
      navigate(`/workspace/${project.id}`);
      toast.success('The fix is ready in a new tab', {
        description: 'Press Enter in the tab to run it.',
      });
    } finally {
      setLaunching(false);
    }
      ...
      <DialogContent className="flex max-h-[85vh] max-w-2xl flex-col gap-0 p-0">
        ...
        <div className="flex min-h-0 flex-1 flex-col gap-2 px-5 py-4">
          <div className="flex items-center gap-2">
            ...
            <RunRecommendationChip state={recommendation} className="ml-auto max-w-[16rem]" />
          </div>
          {failure.isPending ? (
        ...
        <DialogFooter className="flex-row items-center gap-2 border-t border-border/70 px-5 py-3 sm:justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button
              size="sm"
              className="gap-1.5"
              disabled={!prompt || launching || (!suggestion && !cliId)}
              onClick={() =>
                void openFix(
                  suggestion
                    ? {
