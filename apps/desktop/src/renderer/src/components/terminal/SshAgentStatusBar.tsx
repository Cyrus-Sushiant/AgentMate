...
import { CircleCheck, Robot, StopCircle, TriangleAlert, X } from '@/components/icons';
  ...
  return (
    <div
      className={cn(
        ...
        state.phase === 'needs-input' && 'bg-amber-500/10',
        !done && state.phase !== 'needs-input' && state.phase !== 'error' && 'bg-primary/5',
      )}
    >
      {state.phase === 'error' ? (
      ...
      {!done && state.phase !== 'proposed' && state.phase !== 'needs-input' && (
        <Button
          size="sm"
          variant="ghost"
